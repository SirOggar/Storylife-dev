...
package fr.siroggar.storylife.Items.inventory;
...
public class WalletInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == StoryLifeItems.wallet.get()) {
			if (Minecraft.getInstance().screen instanceof GuiwalletScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(31) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != StoryLifeItems.wallet.get();
			}
